...
public class OverLappingErrorTest {
	...
	public void setUp() throws Exception {
		...
		expectedResults.add(new Check[] { Check.GOOD, Check.GOOD, Check.GOOD,
				Check.GOOD });
		partner_list
				.add(new NamedEntityCorrections[] { null, null, null, null });
		...
		expectedResults.add(new Check[] { Check.GOOD, Check.GOOD,
				Check.OVERLAPS, Check.GOOD, Check.GOOD, Check.GOOD });
		...
		doc.add(new DocumentImpl(
				TEXTS[0],
				"http://www.ontologydesignpatterns.org/data/oke-challenge/task-1/sentence-1",
				Arrays.asList((Marking) new NamedEntityCorrections(0, 20,
						"http://dbpedia.org/resource/Florence_May_Harding"),
						(Marking) new NamedEntityCorrections(10, 24,
								"http://dbpedia.org/resource/Sydney"))));
		partner_list.add(new NamedEntityCorrections[] {
				new NamedEntityCorrections(10, 24,
						"http://dbpedia.org/resource/Sydney"), null });

		expectedResults.add(new Check[] { Check.OVERLAPS, Check.GOOD });
		...
		doc.add(new DocumentImpl(
				...
				Arrays.asList(
						...
						(Marking) new NamedEntityCorrections(29, 12,
								"http://dbpedia.org/resource/Bill_Clinton"),
						(Marking) new NamedEntityCorrections(96, 13,
		...
		expectedResults.add(new Check[] { Check.OVERLAPS, Check.OVERLAPS,
				Check.GOOD, Check.GOOD, Check.GOOD, Check.GOOD });

		partner_list.add(new NamedEntityCorrections[] {
				new NamedEntityCorrections(26, 17,
						"http://dbpedia.org/resource/Political_consulting",
						Check.OVERLAPS, new NamedEntityCorrections(29, 12,
								"http://dbpedia.org/resource/Bill_Clinton")),
				new NamedEntityCorrections(29, 12,
						"http://dbpedia.org/resource/Bill_Clinton"), null,
				null, null, null });
	...
	public void test() throws GerbilException {
		...
		for (int i = 0; i < doc.size(); i++) {
			...
			for (int j = 0; j < markings.size(); j++) {
				Assert.assertEquals(expectedResult[j], markings.get(j)
						.getResult());
