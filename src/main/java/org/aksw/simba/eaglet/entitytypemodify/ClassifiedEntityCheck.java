...
public class ClassifiedEntityCheck extends EntityCheck implements ClassifiedSpanMeaning {

    protected BitSet classBits = new BitSet(MarkingClasses.NUMBER_OF_CLASSES);

    public ClassifiedEntityCheck(EntityCheck marking) {
        this(marking.getStartPosition(), marking.getLength(), marking.getUris(), marking.isNamedEntity());
    }

    public ClassifiedEntityCheck(int startPosition, int length, Set<String> uris, boolean value) {
        super(startPosition, length, uris, value);
    }
