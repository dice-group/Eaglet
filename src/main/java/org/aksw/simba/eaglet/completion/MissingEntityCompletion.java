...
public class MissingEntityCompletion implements GoldStandardCompletion {
	...
	public void addMissingEntity(List<List<MeaningSpan>> annotator_result, Document doc) throws GerbilException {
		List<NamedEntityCorrections> entity_list = doc.getMarkings(NamedEntityCorrections.class);
		...
		for (List<NamedEntityCorrections> list : annotatorresult) {
			for (NamedEntityCorrections annotatorentity : list) {
				result = false;
				for (NamedEntityCorrections docentity : entity_list) {
					if ((annotatorentity.getStartPosition() == docentity.getStartPosition())
							&& (annotatorentity.getLength() == docentity.getLength())) {
						result = true;
					}
					if (result == false) {
