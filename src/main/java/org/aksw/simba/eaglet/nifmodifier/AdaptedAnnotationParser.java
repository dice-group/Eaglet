...
public class AdaptedAnnotationParser extends AnnotationParser {
	...
	private Check parseCheckResult(Resource asResource) {
		switch (asResource) {
		case EAGLET.Inserted:
			return Check.INSERTED;
		case EAGLET.Deleted:
			return Check.DELETED;
		case EAGLET.Good:
			return Check.GOOD;
		case EAGLET.NeedToPair:
			return Check.NEED_TO_PAIR;
		case EAGLET.Overlaps:
			return Check.OVERLAPS;
		case EAGLET.Completed:
			return Check.COMPLETED;
		}
		LOGGER.error("Got an unknown matching type: " + asResource);
		return null;
	}
