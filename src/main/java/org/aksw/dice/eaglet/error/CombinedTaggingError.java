...
public class CombinedTaggingError implements ErrorChecker {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(CombinedTaggingError.class);
			...
			if (entities.size() > 0) {
				for (int i = 1; i < entities.size(); ++i) {
					if (entities.get(i).getResult().equals(Check.GOOD)) {
						String substring;
						if ((entities.get(i - 1).getStartPosition() + entities
								.get(i - 1).getLength()) <= entities.get(i)
								.getStartPosition()) {
							substring = text.substring(entities.get(i - 1)
									.getStartPosition()
									+ entities.get(i - 1).getLength(), entities
							...
							if (substring.matches("[\\s]*")) {
								...
								entities.get(i - 1).setResult(
										Check.NEED_TO_PAIR);
								entities.get(i).setError(ErrorType.COMBINED);
								entities.get(i - 1)
										.setError(ErrorType.COMBINED);
							}
